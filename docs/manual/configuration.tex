...
and offset. There is deliberately no built-in default: parameters are
constructed from the capture resolution actually in effect (the helper
\cfg{CalibrationParams::centered(width, height, mm\_per\_px)} maps the
optical centre to the robot origin). A guided calibration wizard is
planned (see the TODO list); until it exists, the parameters in
\cfg{src/vision/calibration.rs} must be adjusted to match the physical
camera mounting.
