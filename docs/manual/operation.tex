...
\begin{enumerate}
  ...
  \item If a part is still held by the gripper, press \emph{Release
    Gripper}. The gripper is \emph{not} released automatically on E-stop:
    right after \cfg{M112} the halted controller would not acknowledge the
    command and the wait would stall recovery, so releasing is left to this
    manual step once the robot is responsive again (typically after
    re-homing).
