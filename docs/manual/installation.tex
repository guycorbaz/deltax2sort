...
\subsection{Hardware}
\begin{itemize}
  \item \textbf{Delta X2 delta robot} (basic kit,
    \href{https://docs.deltaxrobot.com/products/deltax2/deltax2_basic_kit/}{docs.deltaxrobot.com}),
    connected over USB serial.
  \item \textbf{Conveyor belt} with a G-code speed controller, connected over
    USB serial.
  \item \textbf{USB camera} mounted above the belt. The exact model is not
    chosen yet; all capture parameters (resolution, frame rate, pixel
    format) are configurable in the \cfg{[camera]} section (default
    $1280 \times 720$ @ 30\,fps).
